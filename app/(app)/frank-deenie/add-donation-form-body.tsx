...
export interface AddDonationFormBodyProps {
  /** Signal to reset draft state (e.g. on re-open). Changing this value resets the form. */
...
export const AddDonationFormBody = memo(function AddDonationFormBody({
  resetKey,
  selectedYear,
  nameSuggestions,
  onCreated,
    ...
    <form id={formId} className="grid gap-3" onSubmit={handleSubmit}>
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <div className="text-xs font-semibold text-muted-foreground">
          ...
          <button
            ...
            className={`mt-1 flex h-10 w-full cursor-pointer items-center rounded-lg border bg-transparent px-3 text-sm shadow-xs transition-colors hover:bg-muted/40 ${errors.date ? "border-rose-300" : "border-input"}`}
          >
        ...
        <label className="text-xs font-semibold text-muted-foreground">
          ...
          <AmountInput
            ...
            className={`mt-1 h-10 rounded-lg ${errors.amount ? "border-rose-300 focus:border-rose-500" : ""}`}
            required
          />
          {errors.amount && (
        ...
        <label className="text-xs font-semibold text-muted-foreground">
          ...
          <select
            ...
            className="border-input bg-transparent shadow-xs focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] h-10 w-full rounded-lg border px-3 py-1 text-base outline-none md:text-sm mt-1"
          >
      ...
      {renderInlineActions ? (
        <div className="mt-1 flex items-center gap-2">
          ...
          <Button variant="outline" type="button" onClick={onCancel} disabled={isCreating} className="flex-1">
            Cancel
          </Button>
        </div>
      ) : null}
