...
import { processPendingEmailDeliveries } from "@/lib/email-notifications";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isAuthorizedByWorker(request, "EMAIL_WORKER_SECRET")) {
      const context = await requireAuthContext();
      assertRole(context.profile, ["oversight", "admin"]);
    }

    const result = await processPendingEmailDeliveries(admin, { limit });
