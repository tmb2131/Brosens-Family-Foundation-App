...
import { processPendingOrganizationCategoryJobs } from "@/lib/organization-categorization";
import { createAdminClient } from "@/lib/supabase/admin";

function isAuthorizedBySecret(request: NextRequest) {
  const workerSecret = process.env.ORG_CATEGORY_WORKER_SECRET?.trim();
  if (!workerSecret) {
    return false;
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await processPendingOrganizationCategoryJobs(admin, { limit });
